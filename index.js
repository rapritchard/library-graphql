...
const resolvers = {
    ...
    allBooks: (root, args) => {
      ...
      if (args.genre) {
        filteredBooks = filteredBooks.filter((book) =>
          book.genres.find(
            (genre) => genre.toLowerCase() === args.genre.toLowerCase()
          )
        );
      }
  ...
  Mutation: {
    addBook: (root, args) => {
      const updatedBook = { ...args, id: uuid() };
      books = books.concat(updatedBook);

      const isAuthor = authors.find(
        (author) => author.name === updatedBook.author
      );
      if (!isAuthor) {
        const author = { name: updatedBook.author };
